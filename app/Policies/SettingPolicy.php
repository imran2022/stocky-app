...
public function mail_settings(User $user)
{
    ...
    public function business_modules(User $user)
    {
        $modulesPermission = Permission::where('name', 'business_modules')->first();
        $systemPermission = Permission::where('name', 'setting_system')->first();

        if (
            ($modulesPermission && $user->hasRole($modulesPermission->roles)) ||
            ($systemPermission && $user->hasRole($systemPermission->roles))
        ) {
            return true;
        }
