...
use App\Support\Reporting\CashFlowFigures;
  ...
  public function Report_Sales(Request $request)
{
    ...
    $Sales = Sale::select('sales.*')
        ->with('facture.payment_method', 'client', 'warehouse', 'user', 'zone', 'courier', 'seller')
    ...
    public function cash_flow_report(Request $request)
    {
        ...
        // One read of every cash movement; the table and the chart are folded from the same entries.
        $summary = CashFlowFigures::summarise(CashFlowFigures::entries([
            'from' => $from,
            'to' => $to,
            'group_by' => $groupBy,
            ...
            'allowed_warehouses' => $is_all_warehouses ? null : $warehouse_ids,
            'view_records' => (bool) $viewRecords,
            'user_id' => (int) Auth::id(),
        ]));
