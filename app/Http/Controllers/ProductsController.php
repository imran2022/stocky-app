...
class ProductsController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        // 'code' is handled separately below so it can also match variant codes —
        // the generic filter can only compare columns on products itself.
        $columns = [0 => 'name', 1 => 'category_id', 2 => 'brand_id', 3 => 'sub_category_id'];
        $param = [0 => 'like', 1 => '=', 2 => '=', 3 => '='];
            ...
            ->where(function ($query) use ($request) {
                return $query->when($request->filled('search'), function ($query) use ($request) {
                    ...
                    return $query->where('products.name', 'LIKE', "%{$s}%")
                        ...
                        // A variant's own code/name only lives on product_variants,
                        // so without this a scanned variant barcode finds nothing.
                        ->orWhere(function ($q) use ($s) {
                            $q->whereHas('variants', function ($vq) use ($s) {
        ...
        // Code filter: matches the product's own code OR any of its variant codes,
        // so a variant barcode typed/scanned into the Code box finds its parent.
        if ($request->filled('code')) {
            $code = $request->code;
            $filtered->where(function ($q) use ($code) {
                $q->where('products.code', 'LIKE', "%{$code}%")
                    ->orWhereHas('variants', function ($vq) use ($code) {
                        $vq->whereNull('deleted_at')->where('code', 'LIKE', "%{$code}%");
                    });
            });
        }
        ...
        if ($order === 'quantity') {
            $qtySub = product_warehouse::selectRaw('COALESCE(SUM(qte), 0)')
                ->whereColumn('product_warehouse.product_id', 'products.id')
                ->whereNull('deleted_at');
            if ($warehouseId) {
                $qtySub->where('warehouse_id', $warehouseId);
        ...
        foreach ($products as $product) {
            ...
            // Variant products list the parent code first and every variant code
            // under it; other types leave this empty.
            $item['variant_codes'] = [];
            ...
            if ($product->type === 'is_single') {
                ...
                $item['variant_codes'] = $variants->pluck('code')->filter()->values()->all();
                $item['cost'] = $variants
                ...
                $item['price'] = $variants
                    ...
                    ->implode("\n");
                $item['wholesale_price'] = $hasVariantWholesale
                    ? $variants
                        ->map(fn ($v) => number_format((float) $v->wholesale, helpers::price_decimals(), '.', ''))
                        ->implode("\n")
                    : null;
                $item['min_price'] = $hasVariantMinPrice
                    ? $variants
                        ->map(fn ($v) => number_format((float) $v->min_price, helpers::price_decimals(), '.', ''))
                        ->implode("\n")
                    : null;
                $item['unit'] = optional($product->unit)->ShortName;
                ...
                $item['name'] = $product->name; // PLAIN TEXT
                $item['cost'] = number_format((float) $product->cost, helpers::price_decimals(), '.', '');
                // A service is never stocked, so quantity/unit stay blank.
                $item['quantity'] = '----';
                $item['unit'] = '----';
                $item['price'] = number_format((float) $product->price, helpers::price_decimals(), '.', '');
                $item['wholesale_price'] = number_format((float) $product->wholesale_price, helpers::price_decimals(), '.', '');
    ...
    private function storeVariantImage($file): string
    {
        ...
        try {
            ...
            helpers::apply_product_image_resize($image_resize)
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \DB::transaction(function () use ($request) {
                ...
                $Product->is_returnable = filter_var($request->input('is_returnable', true), FILTER_VALIDATE_BOOLEAN) ? 1 : 0;
                ...
                // Multi-Pack Selling: persist selling packs. Runs after the
                ...
                if ($warehouseIds) {
                    ...
                    // fetch variants if needed — ordered by id so positions match
                    // the request's variants array (bulk insert preserves order)
                    $variants = $isVariant
                    ...
                    $variantOpening = json_decode($request->input('variant_opening', '[]'), true) ?: [];
                        ...
                        if ($isVariant) {
                            foreach ($variants as $vIndex => $variant) {
                                $insertRows[] = [
                                    ...
                                    'qte' => (float) ($variantOpening[$wid][$vIndex] ?? 0),
                                ];
                            }
                        } else {
                            $insertRows[] = [
                    ...
                    // Same "virtual" opening-stock adjustments for variant products:
                    // one header per warehouse, one detail row per variant with stock.
                    if ($isVariant && $manage_stock && ! empty($variantOpening)) {
                        foreach ($warehouseIds as $wid) {
                            $detailRows = [];
                            foreach ($variants as $vIndex => $variant) {
                                $q = (float) ($variantOpening[$wid][$vIndex] ?? 0);
                                if ($q > 0) {
                                    $detailRows[] = ['variant_id' => $variant->id, 'qty' => $q];
                                }
                            }
                            if (empty($detailRows)) {
                                continue;
        ...
        try {
            ...
            \DB::transaction(function () use ($request, $id) {
                ...
                if ($currentImage && $request->hasFile('image')) {
                    ...
                    // Downscale per the Resize Product Images setting (off = original size)
                    $image_resize = Image::make($image->getRealPath());
                    helpers::apply_product_image_resize($image_resize)->save($path.'/'.$filename);
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        $Product = Product::with(['category', 'categories', 'subCategory', 'subcategories', 'images', 'sizeGuide'])
            ->where('deleted_at', '=', null)
            ->findOrFail($id);
        ...
        $item['brand'] = $Product['brand'] ? $Product['brand']->name : 'N/D';
        $item['size_guide'] = $Product->sizeGuide ? [
            'id' => $Product->sizeGuide->id,
            'name' => $Product->sizeGuide->name,
            'image' => $Product->sizeGuide->image,
            'columns' => is_array($Product->sizeGuide->columns) ? $Product->sizeGuide->columns : [],
            'rows' => is_array($Product->sizeGuide->rows) ? $Product->sizeGuide->rows : [],
        ] : null;
        ...
        // Multi-Pack Selling: list the product's packs when the feature is enabled
        // (single and variant products). Empty otherwise, so the detail UI hides
        // the section. Variant packs carry their variant's name for display.
        $item['packs'] = [];
        $setting = Setting::whereNull('deleted_at')->first();
        if ((bool) ($setting->enable_multi_pack_selling ?? false) && in_array($Product->type, ['is_single', 'is_variant'], true)) {
            $packVariantNames = $Product->type == 'is_variant'
                ? ProductVariant::where('product_id', $id)->whereNull('deleted_at')->pluck('name', 'id')
                : collect();
            foreach (ProductPack::where('product_id', $id)
                ->whereNull('deleted_at')
                ->orderBy('product_variant_id')
                ->orderBy('is_default', 'desc')
                ->orderBy('id')
                ->get() as $pack) {
                if ($pack->product_variant_id && ! $packVariantNames->has($pack->product_variant_id)) {
                    continue; // pack of a deleted variant
                }
                $item['packs'][] = [
                    'id' => $pack->id,
                    'name' => $pack->name,
                    'multiplier' => (float) $pack->multiplier,
                    'price' => number_format((float) $pack->price, helpers::price_decimals(), '.', ','),
                    'is_active' => (bool) $pack->is_active,
                    'is_default' => (bool) $pack->is_default,
                    'variant_name' => $pack->product_variant_id ? $packVariantNames[$pack->product_variant_id] : null,
                ];
            }
        }
    ...
    public function Products_by_Warehouse(request $request, $id)
    {
                ...
                return $query->where('warehouse_id', $id)
                    ...
                    ->where(function ($query) use ($request) {
                        if ($request->stock == '1' && $request->product_service == '1') {
                            ...
                            // include stock-managed products and non-managed (service) products.
                            return $query;
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            // variant code above, so without this searching by the main code
            // could never match a variant.
    ...
    public function show_product_data($id, $variant_id, $warehouse_id = null)
    {
        ...
        // for single products the flat list, for variant products the packs of
        // the requested variant. Empty otherwise, so non-pack flows are unchanged.
        $item['packs'] = [];
        $setting = Setting::whereNull('deleted_at')->first();
        if ((bool) ($setting->enable_multi_pack_selling ?? false)) {
            $packQuery = null;
            if ($Product_data['type'] == 'is_single') {
                $packQuery = ProductPack::where('product_id', $id)
                    ->whereNull('product_variant_id');
            } elseif ($Product_data['type'] == 'is_variant' && $variant_id && $variant_id != 'null') {
                $packQuery = ProductPack::where('product_id', $id)
    ...
    public function Products_Alert(request $request)
    {
        ...
        // Whole-set stats for the report's summary cards (before pagination).
        $stats = [
            'total' => count($data),
            'out_of_stock' => count(array_filter($data, fn ($r) => (float) $r['quantity'] <= 0)),
            'critical' => count(array_filter($data, fn ($r) => (float) $r['quantity'] > 0
                && (float) $r['stock_alert'] > 0
                && (float) $r['quantity'] / (float) $r['stock_alert'] <= 0.34)),
            'warehouses' => count(array_unique(array_column($data, 'warehouse'))),
        ];
    ...
    public function create(Request $request)
    {
        ...
        // create form. Controlled from System Settings; defaults to true.
        $setting = Setting::whereNull('deleted_at')->first();
    ...
    public function Get_element_barcode(Request $request)
    {
        ...
        $label_settings = $setting && $setting->barcode_label_settings
            ? json_decode($setting->barcode_label_settings, true) : null;
    ...
    // ---------------- Direct label printing (raw TSPL, no browser dialog) ---------------\\

    public function Print_Labels_Direct(Request $request)
    {
        $this->authorizeForUser($request->user('api'), 'barcode', Product::class);
    ...
    public function edit(Request $request, $id)
    {
        ...
        // Multi-Pack Selling: per-product selling packs (empty for products that
        // don't use them). Variant products carry per-variant pack lists, so each
        // row exposes its variant id + name for the form to group by.
        $item['packs'] = [];
        $packVariantNames = $Product->type == 'is_variant'
            ? ProductVariant::where('product_id', $id)->whereNull('deleted_at')->pluck('name', 'id')
            : collect();
        foreach (ProductPack::where('product_id', $id)->whereNull('deleted_at')->orderBy('is_default', 'desc')->orderBy('id')->get() as $pack) {
            $item['packs'][] = [
                ...
                'is_active' => (bool) $pack->is_active,
                'is_default' => (bool) $pack->is_default,
                'product_variant_id' => $pack->product_variant_id,
                'variant_name' => $pack->product_variant_id ? ($packVariantNames[$pack->product_variant_id] ?? null) : null,
            ];
        }
        ...
        $item['is_returnable'] = $Product->is_returnable === null ? true : (bool) $Product->is_returnable;
        $item['is_active'] = $Product->is_active ? true : false;

        $item['is_classified'] = (bool) $Product->is_classified;
        $item['is_preorder'] = (bool) $Product->is_preorder;
        $item['preorder_always'] = (bool) $Product->preorder_always;
    ...
    /**
     * Normalize the incoming tags payload (JSON string or array) to a clean string list.
     */
     ...
     */
    protected function normalizeFaqs($raw): array
    {
        $val = is_string($raw) ? (json_decode($raw, true) ?: []) : (is_array($raw) ? $raw : []);
    ...
    /**
     * Multi-Pack Selling: persist / reconcile a product's selling packs.
     *
     * Only runs when the global feature toggle is on; otherwise existing packs
     * are left untouched so turning the feature off never destroys config. The
     * incoming "packs" payload is a JSON array of
     * { id?, name, multiplier, price, is_active, is_default,
     *   product_variant_id?, variant_name? }.
     *
     * Single products keep one flat pack list (product_variant_id NULL).
     * Variant products carry one pack list per variant: each row is resolved to
     * its variant by product_variant_id (existing variants) or by variant_name
     * (variants created in the same submit) — must be called AFTER variants are
     * persisted. Exactly one default pack (multiplier 1, always active) is
     * guaranteed per scope that has rows. Packs are matched by id: existing
     * rows are updated, new rows inserted, removed rows soft-deleted (which
     * also drops packs of deleted variants).
     */
    protected function syncProductPacks(Request $request, Product $product): void
    {
        ...
        if (! in_array($product->type, ['is_single', 'is_variant'], true) || ! $request->has('packs')) {
            return;
        }
        ...
        // Variant products: map submitted rows to their persisted variants.
        $isVariantProduct = $product->type === 'is_variant';
        $variants = $isVariantProduct
            ? ProductVariant::where('product_id', $product->id)->whereNull('deleted_at')->get()
            : collect();
        $variantIdByName = $variants->pluck('id', 'name');
        $variantPriceById = $variants->pluck('price', 'id');

        // Sanitize incoming rows and group them per scope: NULL for a single
        // product, the variant id for variant products. Drop anything invalid
        // or pointing at a variant that no longer exists.
        $groups = [];
        $hasDefault = [];
        foreach ($packs as $p) {
            $variantId = null;
            if ($isVariantProduct) {
                $vid = ! empty($p['product_variant_id']) ? (int) $p['product_variant_id'] : null;
                $vname = isset($p['variant_name']) ? (string) $p['variant_name'] : '';
                if ($vid && $variantPriceById->has($vid)) {
                    $variantId = $vid;
                } elseif ($vname !== '' && $variantIdByName->has($vname)) {
                    $variantId = (int) $variantIdByName[$vname];
                } else {
                    continue; // orphan row — its variant was removed or never saved
                }
            }
            $scope = $variantId ?? 0;
            ...
            if ($isDefault) {
                if (! empty($hasDefault[$scope])) {
                    continue; // only the first default wins per scope
                }
                // Force the canonical default: multiplier 1, active, named.
                $multiplier = 1.0;
                $name = $name !== '' ? $name : 'Default';
                // Default pack sells in the base unit; fall back to the variant
                // (or product) price when no explicit price was set so it never
                // sells at 0.
                if ($price <= 0) {
                    $price = $variantId !== null
                        ? (float) ($variantPriceById[$variantId] ?? 0)
                        : (float) $product->price;
                }
                $hasDefault[$scope] = true;
        ...
        // Guarantee a default pack per scope that has rows. Single products
        // always keep their scope so the base-unit pack survives an empty list.
        if (! $isVariantProduct && ! isset($groups[0])) {
            $groups[0] = [];
        }
        foreach ($groups as $scope => $rows) {
            if (empty($hasDefault[$scope])) {
                $variantId = $isVariantProduct ? (int) $scope : null;
                array_unshift($groups[$scope], [
                    'id' => null,
                    'product_variant_id' => $variantId,
                    'name' => 'Default',
                    'multiplier' => 1.0,
                    'price' => max(0, $variantId !== null
                        ? (float) ($variantPriceById[$variantId] ?? 0)
                        : (float) $product->price),
                    'is_active' => true,
                    'is_default' => true,
        ...
        foreach ($groups as $rows) {
            foreach ($rows as $row) {
                ...
                $pack->product_variant_id = $row['product_variant_id'];
                $pack->name = $row['name'];
        ...
        // Soft-delete rows that were removed in the UI (or whose variant was
        // deleted / whose product changed type).
