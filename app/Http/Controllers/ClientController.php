...
class ClientController extends BaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($request, $id, $isRoyaltyEligible) {
            ...
            // dedicated endpoints (clients_without_ecommerce / ecommerce_clients),
            // and ecommerce_clients.email is NOT NULL — a plain POS customer
            // edit with an empty email used to crash on that constraint.
            $ecommerceClient = EcommerceClient::where('client_id', $id)
                ->whereNull('deleted_at')
                ->first();

            if ($ecommerceClient) {
                if ($request->filled('username')) {
                    $ecommerceClient->username = $request->input('username');
                }
                // Keep the existing login email when the edit form left it blank.
                if ($request->filled('email')) {
                    $ecommerceClient->email = $request->input('email');
                }
                if ($request->filled('password')) {
                    $ecommerceClient->password = Hash::make($request->input('password'));
                }
                $ecommerceClient->save();
            }
        });
    ...
    public function clientBrief(Request $request, $id)
    {
        ...
        $client->wallet_enabled = $walletService->enabled();
        $client->wallet_balance = $client->wallet_enabled ? $walletService->balanceFor($client->id) : 0;
