import{s as ut,bD as ct,bE as pt,r as c,b_ as L,o as mt,j as y,c as p,e as $,m as s,p as i,q as v,l as o,x as d,bV as a,F as V,z as j,P as D,k as n,bB as vt,Q as _t,y as lt,v as yt,bU as H,A as J,bJ as bt,bO as ft}from"../app.ClxNdhK0.js";import{_ as ht}from"./PageHeader.CCahPGNE.js";import{_ as gt}from"./DateRangePicker.Ct7cT3LO.js";import{_ as xt}from"./ViewCurrencySelect.CRLy_WBP.js";import{e as wt,a as $t}from"./exporters.DYrj6VjN.js";import{P as Tt}from"./PrinterOutlined.x_5bRdRm.js";import{F as kt}from"./FileExcelOutlined.uo4o2-Tx.js";import{F as St}from"./FilePdfOutlined.DB9kKRIc.js";import{N as Dt}from"./NumberOutlined.CYbXlyvp.js";import{D as Ct}from"./DollarOutlined.CNR6cAN2.js";const Pt={class:"page"},qt={class:"kpi-inner"},zt={class:"kpi-text"},It={class:"kpi-value"},Ot={key:1,class:"summary-table-wrap"},Nt={class:"summary-table"},Rt={style:{width:"55%"}},Et={class:"num",style:{width:"15%"}},Ut={class:"num",style:{width:"30%"}},Yt={class:"group-row"},Ft={colspan:"3"},At={class:"num"},Gt={class:"num"},Bt={class:"subtotal-row"},Mt={class:"num bold"},Vt={class:"num bold"},Qt={class:"num bold"},Lt={class:"grand-row"},jt={class:"num bold"},Ht={class:"num bold"},Jt={class:"num bold"},Wt={__name:"ProductsSoldSummaryReport",setup(Kt){const{t:g}=ct(),{money:q,number:ot}=pt(),C=c([L().startOf("day"),L()]),R=c(void 0),E=c(void 0),U=c(void 0),Y=c(void 0),F=c(""),Q=c(80),x=c(!0),A=c(""),z=c([]),_=c({quantity:0,total:0,items:0}),r=c({}),f=c({}),W=c([]),K=c([]),X=c([]),st=J(()=>[{value:"completed",label:a("Completed","Completed")},{value:"pending",label:a("Pending","Pending")},{value:"ordered",label:a("Ordered","Ordered")}]),Z=e=>(e||[]).map(t=>({value:t.id,label:t.name})),nt=J(()=>(K.value||[]).map(e=>({value:e.id,label:e.username})));function T(e){const t=Number(e||0);return Number.isInteger(t)?String(t):t.toFixed(2)}function G(e){return e.category_id===null?a("Uncategorized","Uncategorized"):e.category_name}const rt=J(()=>[{key:"items",label:a("Items_Sold","Items sold"),value:_.value.items,icon:bt,color:"#1677ff",tint:"rgba(22, 119, 255, 0.12)"},{key:"qty",label:a("Total_Qty","Total qty"),value:T(_.value.quantity),icon:Dt,color:"#13c2c2",tint:"rgba(19, 194, 194, 0.12)"},{key:"total",label:a("Grand_Total","Grand total"),value:q(_.value.total),icon:Ct,color:"#6d28d9",tint:"rgba(109, 40, 217, 0.12)"},{key:"trans",label:`${a("Transactions","Transactions")} / ${a("Cust_Count","Customers")}`,value:`${r.value.transaction_count??0} / ${r.value.customer_count??0}`,icon:ft,color:"#22c55e",tint:"rgba(34, 197, 94, 0.12)"}]),it=()=>{var e,t;return{...(e=C.value)!=null&&e[0]?{from:C.value[0].format("YYYY-MM-DD")}:{},...(t=C.value)!=null&&t[1]?{to:C.value[1].format("YYYY-MM-DD")}:{},...R.value?{warehouse_id:R.value}:{},...E.value?{user_id:E.value}:{},...U.value?{category_id:U.value}:{},...Y.value?{statut:Y.value}:{},...F.value?{search:F.value}:{}}};async function k(){x.value=!0;try{const e=await yt.get("report/products_sold_summary",it());z.value=Array.isArray(e.categories)?e.categories:[],_.value=e.totals||{quantity:0,total:0,items:0},r.value=e.meta||{},f.value=e.company||{},W.value=e.warehouses||[],K.value=e.users||[],X.value=e.categories_list||[]}catch(e){H.error((e==null?void 0:e.message)||"Error")}finally{x.value=!1}}function tt(){const e=[];return z.value.forEach(t=>{t.items.forEach(m=>e.push({category_name:G(t),product_code:m.product_code||"",product_name:m.product_name,unit:m.unit||"",quantity:m.quantity,total:m.total})),e.push({category_name:G(t),product_code:"",product_name:a("Sub_Total","Sub Total"),unit:"",quantity:t.sub_total_quantity,total:t.sub_total})}),e.length&&e.push({category_name:"",product_code:"",product_name:a("Grand_Total","Grand Total"),unit:"",quantity:_.value.quantity,total:_.value.total}),e}const et=()=>[{title:g("Categorie"),dataIndex:"category_name"},{title:g("Code"),dataIndex:"product_code"},{title:g("ProductName"),dataIndex:"product_name"},{title:a("Unit","Unit"),dataIndex:"unit"},{title:g("Quantity"),dataIndex:"quantity"},{title:g("Total"),dataIndex:"total",exportValue:e=>q(e.total)}];async function at(e){A.value=e;try{const t=`products-sold-summary_${r.value.from}_${r.value.to}`;e==="excel"?await wt(t,et(),tt()):await $t(a("products_sold_summary","Products Sold Summary"),et(),tt())}catch(t){H.error((t==null?void 0:t.message)||"Export failed")}finally{A.value=""}}function dt(){const e=window.open("","_blank","width=420,height=640");if(!e){H.error("Please allow popups to print");return}const t=u=>String(u??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;"),m=Number(Q.value)||80,S=u=>ot(u),I=r.value.day_status==="open"?a("Day_is_still_opened","Day is still opened"):r.value.day_status==="closed"?a("Day_is_closed","Day is closed"):"",h=(u,l)=>`<div class="kv"><span class="k">${t(u)}</span><span class="s">:</span><span class="v">${t(l)}</span></div>`;let b="";z.value.forEach(u=>{b+=`<div class="cat">${t(G(u))}</div>`,b+='<table class="items">',u.items.forEach(l=>{b+=`<tr>
      ...
      </tr>`,b+="</table>"});const P=r.value.from===r.value.to?r.value.from:`${r.value.from} — ${r.value.to}`,B=L().format("DD-MM-YYYY HH:mm:ss"),O=e.document;O.open(),O.write(`<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
