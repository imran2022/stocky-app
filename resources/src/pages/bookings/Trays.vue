<template>
  <div class="page">
    <PageHeader :title="$t('Trays')" :breadcrumb="[$t('Bookings'), $t('Trays')]">
      <template #actions>
...
<script setup>
/** Trays: GET trays → {trays, totalRows}; JSON {name, description}. */
...
const { t } = useI18n();
const crud = useCrudTable('trays', { rowsKey: 'trays' });
...
async function submit() {
  ...
  try {
    if (editMode.value) {
      await http.put(`trays/${form.value.id}`, body);
      message.success(t('Successfully_Updated'));
    } else {
      await http.post('trays', body);
