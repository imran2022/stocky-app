<template>
  <div class="page">
    <PageHeader title="Supplier Statement" :breadcrumb="[$t('Suppliers'), 'Supplier Statement']">
      <template #actions>
        <a-button @click="$router.push(`/suppliers/${id}/details`)">
        ...
        <div class="hero-main">
          ...
          <div class="hero-info">
            <div class="hero-name-row">
              <span class="hero-name">{{ provider.name || '-' }}</span>
            </div>
            <div class="hero-meta">
              <span v-if="provider.email" class="meta-chip"><MailOutlined /> {{ provider.email }}</span>
              <span v-if="provider.phone" class="meta-chip"><PhoneOutlined /> {{ provider.phone }}</span>
              <span v-if="providerAddress" class="meta-chip"><EnvironmentOutlined /> {{ providerAddress }}</span>
            </div>
          </div>
        </div>
      </a-card>
...
<script setup>
/**
 * Supplier Statement mirrors CustomerStatement.vue: one running balance,
 * the same date controls and the same PDF/Excel actions. Supplier purchases
 * are debits; payments and returns are credits; return refunds are debits.
 */
...
// Same 90-day default as Customer Statement. ProviderStatementService folds
// older activity into the opening balance, so the closing figure stays true.
...
const providerAddress = computed(() =>
  [provider.value.adresse, [provider.value.city, provider.value.country].filter(Boolean).join(', ')]
    .filter(Boolean).join(' — ')
);
...
const TYPE_COLORS = {
  purchase: 'blue', purchase_payment: 'green', opening: 'purple', opening_payment: 'green',
  purchase_return: 'orange', return_refund: 'red',
};
const TYPE_LABELS = {
  purchase: 'Purchase', purchase_payment: 'Payment', opening: 'Opening_Balance',
  opening_payment: 'Opening_Balance_Payment', purchase_return: 'Purchase_Return',
...
async function fetchStatement() {
  ...
  try {
    const res = await http.get(`providers/${id}/statement`, {
...
async function downloadPdf() {
  ...
  try {
    await http.download(`providers/${id}/statement/pdf${statementQuery()}`, `supplier_statement_${id}.pdf`);
  } catch (e) {
...
async function downloadExcel() {
  ...
  try {
    await http.download(`providers/${id}/statement/excel${statementQuery()}`, `supplier_statement_${id}.xlsx`);
  } catch (e) {
    message.error(t('Failed_to_export'));
  } finally {
    downloading.value = null;
