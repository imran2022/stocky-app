<template>
  <div class="page">
    <PageHeader :title="$t('Expense_Report')" :breadcrumb="[$t('Reports'), $t('Expense_Report')]">
      <template #actions>
        <a-space wrap>
    ...
    <a-row :gutter="[16, 16]" style="margin-bottom: 16px">
      ...
      <a-col :xs="24" :xl="15">
        <a-card size="small" class="chart-card" :title="$t('Expenses_over_time')">
          <ReportChart
            :data="timeseries"
            :fields="[{ key: 'amount', label: t('Total_Expenses') }]"
    ...
    <DataTable :crud="crud" :columns="columns">
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'total_expenses'">
          <strong style="color: #f43f5e">{{ money(record.total_expenses) }}</strong>
        </template>
      ...
      <template #summary>
        <a-table-summary-row>
          ...
          <a-table-summary-cell :index="2" align="right">
            <b style="color: #f43f5e">{{ money(kpis.total || 0) }}</b>
          </a-table-summary-cell>
...
<script setup>
/**
 * Expense report — analytics view over GET report/expenses_report:
 * {reports (grouped by category, paginated), totalRows, kpis, timeseries,
 * warehouses}. KPIs and the daily trend cover the whole filtered set; the
 * donut shows the loaded categories' shares (top 7 + Other).
 */
...
import {
  ...
  WalletOutlined, FileTextOutlined, TagsOutlined, CalendarOutlined,
...
// Payload: { reports, totalRows, kpis, timeseries, warehouses }
const crud = useCrudTable('report/expenses_report', {
  rowsKey: 'reports',
...
const shareOf = r => {
  ...
  return total > 0 ? +(Number(r.total_expenses) / total * 100).toFixed(1) : 0;
};
...
const kpiTiles = computed(() => {
  ...
  return [
    { label: t('Total_Expenses'), value: money(n('total')), icon: WalletOutlined, color: '#f43f5e', tint: 'rgba(244, 63, 94, 0.12)' },
...
const donutRows = computed(() => {
  const rows = [...crud.rows.value]
    .map(r => ({ name: r.category_name, value: Number(r.total_expenses) || 0 }))
...
const columns = computed(() => [
  { title: t('Expense_Category'), dataIndex: 'category_name', key: 'category_name' },
  { title: t('Entries'), dataIndex: 'expenses_count', key: 'expenses_count', align: 'right' },
  { title: t('Total_Expenses'), dataIndex: 'total_expenses', key: 'total_expenses', align: 'right', exportValue: r => money(r.total_expenses) },
...
async function allRows() {
  ...
  const data = await http.get('report/expenses_report', { ...filterParams(), page: 1, limit: -1, search: crud.search.value });
  return data.reports || [];
}
...
async function exportList(kind) {
  ...
  try {
    ...
    if (kind === 'xlsx') await exportExcel('Expense_report', columns.value, rows);
    else await exportPdf(t('Expense_Report'), columns.value, rows);
  } finally {
...
async function doPrint() {
  ...
  const rows = await allRows();
  printRows(t('Expense_Report'), columns.value, rows, { win });
}
