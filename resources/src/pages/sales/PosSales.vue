<template>
  <div class="page">
    <PageHeader title="POS Sales" :breadcrumb="[$t('Sales'), 'POS Sales']">
      <template #actions>
        <a-space wrap>
...
const filterParams = () => ({
  ...
  // Fixed, not user-changeable — this page only ever shows sales created
  // through the POS register (SalesController@index filters on Sale.is_pos).
  is_pos: 1,
});
...
async function exportList(kind) {
  ...
  try {
    ...
    if (kind === 'xlsx') await exportExcel('pos_sales', columns.value, rows);
    else await exportPdf('POS Sales', columns.value, rows);
  } catch (e) {
