{{--
    "Modern" Purchase Payment Receipt layout — Matching modern invoice design standard.
--}}
@php
    ...
    $pdfT = \App\Models\PdfTemplate::settingsFor('payment');
@endphp
<!DOCTYPE html>
...
<body>
    ...
    <table style="margin-bottom: 20px;">
        <tr>
            <td style="width: 45%; vertical-align: top;">
                <span class="label">{{ __('pdf.paid_to') }}</span>
                <div style="font-size: 9.5pt; font-weight: bold; color: #1e293b;">{{$payment['supplier_name']}}</div>
                @if(!empty($payment['supplier_adr']))
                <div style="font-size: 8.5pt; color: #64748b; margin-top: 2px;">{{$payment['supplier_adr']}}</div>
                @endif
                @if(!empty($payment['supplier_phone']))
                <div style="font-size: 8.5pt; color: #64748b;">{{ __('pdf.phone') }}: {{$payment['supplier_phone']}}</div>
                @endif
                @if(!empty($payment['supplier_email']))
                <div style="font-size: 8.5pt; color: #64748b;">{{ __('pdf.email') }}: {{$payment['supplier_email']}}</div>
                @endif
            </td>
    ...
    <table class="receipt-table" style="margin-bottom: 20px;">
        <thead>
            <tr>
                <th style="width: 50%;">{{ __('pdf.purchase_reference') }}</th>
        ...
        <tr>
            <td style="width: 100%;">
                <div class="info-card" style="text-align: center; background: #f8fafc; padding: 15px;">
                    <div style="font-size: 8pt; color: #64748b; text-transform: uppercase; font-weight: bold; margin-bottom: 4px;">{{ __('pdf.total_amount_paid') }}</div>
    ...
    <div style="margin-top: 15px; page-break-inside: avoid; break-inside: avoid;">
        ...
        <div style="text-align: center;">
            <div style="font-size: 9.5pt; font-weight: bold; color: #0f172a; text-transform: uppercase; letter-spacing: 0.5px;">{{ __('pdf.payment_processed') }}</div>
